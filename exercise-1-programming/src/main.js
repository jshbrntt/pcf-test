...
const EntryPattern = new RegExp(
    `^(?<controlCharacter>[${Object.values(ControlCharacter).join('')}])?(?<value>.+)$`
);
...
const parseGrid = gridString => {
    try {
        ...
        const entries = rows.map(row => row.split(/ +/));
...
const cursorIsWithinGridBounds = (cursor, grid) => cursor.x < grid[0].length && cursor.y < grid.length;
